...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(
		&params.rawConfig.Bridge.Enable,
		bridgeFlag,
		defaultConfig.Bridge.Enable,
		"Sets the flag indicating that the server should use Bridge. Defaults to false",
	)

	cmd.Flags().StringVar(
		&params.rawConfig.Bridge.RootchainURL,
		bridgeRootchainURLFlag,
		defaultConfig.Bridge.RootchainURL,
		"Sets the source chain URL",
	)

	cmd.Flags().StringVar(
		&params.rawConfig.Bridge.RootchainContract,
		bridgeRootchainContractFlag,
		defaultConfig.Bridge.RootchainContract,
		"Sets the source chain contract address",
	)

	cmd.Flags().Uint64Var(
		&params.rawConfig.Bridge.Confirmations,
		bridgeRootchainConfirmationsFlag,
		defaultConfig.Bridge.Confirmations,
		"Sets the required confirmations on root chain",
	)
