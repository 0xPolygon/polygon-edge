...
func (p *serverParams) initRawParams() error {
	...
	if err := p.initBridgeParams(); err != nil {
		return err
	}
...
func (p *serverParams) initBridgeParams() error {
	...
	if p.bridgeConfig.Enable {
		...
		if p.bridgeConfig.RootChainURL, err = url.Parse(p.rawConfig.Bridge.RootchainURL); err != nil {
			return err
		}

		p.bridgeConfig.RootChainContract = types.StringToAddress(p.rawConfig.Bridge.RootchainContract)

		p.bridgeConfig.Confirmations = p.rawConfig.Bridge.Confirmations
	}
