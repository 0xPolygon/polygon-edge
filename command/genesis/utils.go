...
func parseBurnContractInfo(burnContractInfoRaw string) (uint64, types.Address, types.Address, error) {
	...
	if err != nil {
		return 0, types.ZeroAddress, types.ZeroAddress, fmt.Errorf("failed to parse amount %s: %w", blockRaw, err)
	}

	contractAddress := burnContractParts[1]
	if err = types.IsValidAddress(contractAddress); err != nil {
		return 0, types.ZeroAddress, types.ZeroAddress,
			fmt.Errorf("failed to parse contract address %s: %w", contractAddress, err)
	}

	if len(burnContractParts) == 2 {
		return block.Uint64(), types.StringToAddress(contractAddress), types.ZeroAddress, nil
	}
