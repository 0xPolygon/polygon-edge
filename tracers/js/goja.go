...
func fromBuf(vm *goja.Runtime, bufType goja.Value, buf goja.Value, allowString bool) ([]byte, error) {
	...
	case "String":
		...
		return types.FromHex(obj.String()), nil
	case "Array":
		var b []byte
...
type jsTracer struct {
	...
	transition        *state.Transition     // just like evm
...
func (t *jsTracer) CaptureStart(txr interface{}, from types.Address, to types.Address, create bool, input []byte, gas uint64, value *big.Int) {
	// t.env = env
	...
	t.transition = txn
	db := &dbObj{db: *t.transition.Txn(), vm: t.vm, toBig: t.toBig, toBuf: t.toBuf, fromBuf: t.fromBuf}
	...
	t.ctx["gasPrice"] = t.vm.ToValue(t.transition.GetTxContext().GasPrice)
	...
	t.ctx["block"] = t.vm.ToValue(t.transition.GetTxContext().Number)
	// Update list of precompiles based on current block
	// rules := env.ChainConfig().Rules(t.transition.Block().Number(), env.Context.Random != nil)
	// t.activePrecompiles = vm.ActivePrecompiles(rules)
	t.ctx["intrinsicGas"] = t.vm.ToValue(t.gasLimit - gas)
}

// CaptureState implements the Tracer interface to trace a single step of VM execution.
func (t *jsTracer) CaptureState(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, rData []byte, depth int, err error) {
	if !t.traceStep {
		return
	}
	if t.err != nil {
...
func (t *jsTracer) CaptureFault(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, depth int, err error) {
	if t.err != nil {
		return
	}
...
func (t *jsTracer) CaptureEnter(typ int, from types.Address, to types.Address, input []byte, gas uint64, value *big.Int) {
	...
	t.frame.typ = evm.OpCode(typ).String()
	t.frame.from = from
	t.frame.to = to
	t.frame.input = types.CopyBytes(input)
...
func (t *jsTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	t.frameResult.output = types.CopyBytes(output)
...
func (t *jsTracer) setBuiltinFunctions() {
	...
	vm.Set("toContract2", func(from goja.Value, salt string, initcode goja.Value) goja.Value {
		...
		code = types.CopyBytes(code)
		codeHash := crypto.Keccak256(code)
		b := crypto.CreateAddress2(addr, types.HexToHash(salt), codeHash).Bytes()
		res, err := t.toBuf(vm, b)
		if err != nil {
			vm.Interrupt(err)
			return nil
