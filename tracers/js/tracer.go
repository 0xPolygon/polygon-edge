...
import (
	...
	tracers2 "github.com/0xPolygon/polygon-edge/tracers"
	"github.com/0xPolygon/polygon-edge/tracers/js/internal/tracers"
...
func init() {
	...
	// 记录是否执行
	// fmt.Printf("-------== js.tracer is running ==--------")
...
func popSlice(ctx *duktape.Context) []byte {
	blob := types.CopyBytes(makeSlice(ctx.GetBuffer(-1)))
	ctx.Pop()
	return blob
}
...
func (dw *dbWrapper) pushObject(vm *duktape.Context) {
	...
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		pushBigInt(dw.db.GetBalance(types.BytesToAddress(popSlice(ctx))), ctx)
		return 1
	})
	...
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		ctx.PushInt(int(dw.db.GetNonce(types.BytesToAddress(popSlice(ctx)))))
		return 1
	})
	...
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		code := dw.db.GetCode(types.BytesToAddress(popSlice(ctx)))
	...
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		...
		state := dw.db.GetState(types.BytesToAddress(addr), types.BytesToHash(hash))
	...
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		ctx.PushBoolean(dw.db.Exist(types.BytesToAddress(popSlice(ctx))))
		return 1
	})
...
// onto the VM stack.   is this important to tracer work?
...
type jsTracer struct {
	...
	activePrecompiles []types.Address // Updated on CaptureStart based on given rules  这个变量没有用到？？
...
func newJsTracer(code string, ctx *tracers2.Context) (tracers2.Tracer, error) {
	...
	tracer.vm.PushGlobalGoFunction("toWord", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-1); ptr != nil {
			word = types.BytesToHash(makeSlice(ptr, size))
		} else {
			word = types.HexToHash(ctx.GetString(-1))
		}
		ctx.Pop()
	...
	tracer.vm.PushGlobalGoFunction("toAddress", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-1); ptr != nil {
			addr = types.BytesToAddress(makeSlice(ptr, size))
		} else {
			addr = types.HexToAddress(ctx.GetString(-1))
		}
		ctx.Pop()
	...
	tracer.vm.PushGlobalGoFunction("toContract", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-2); ptr != nil {
			from = types.BytesToAddress(makeSlice(ptr, size))
		} else {
			from = types.HexToAddress(ctx.GetString(-2))
		}
	...
	tracer.vm.PushGlobalGoFunction("toContract2", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-3); ptr != nil {
			...
			from = types.HexToAddress(ctx.GetString(-3))
		}
		// Retrieve salt hex string from js stack
		salt := types.HexToHash(ctx.GetString(-2))
		// Retrieve code slice from js stack
		var code []byte
		if ptr, size := ctx.GetBuffer(-1); ptr != nil {
			code = types.CopyBytes(makeSlice(ptr, size))
		} else {
			code = types.FromHex(ctx.GetString(-1))
		}
	...
	tracer.vm.PushGlobalGoFunction("isPrecompiled", func(ctx *duktape.Context) int {
		addr := types.BytesToAddress(popSlice(ctx))
...
func (jst *jsTracer) CaptureStart(txr interface{}, from types.Address, to types.Address, create bool, input []byte, gas uint64, value *big.Int) {
	...
	jst.ctx["input"] = input
	jst.ctx["gas"] = gas
	jst.ctx["gasPrice"] = jst.transition.GetTxContext().GasPrice
	jst.ctx["value"] = value

	// Initialize the context
	jst.ctx["block"] = jst.transition.Block()
	jst.dbWrapper.db = *jst.transition.Txn()
	// Update list of precompiles based on current block
	//rules := env.ChainConfig().Rules(env.Context.BlockNumber, env.Context.Random != nil)
	//jst.activePrecompiles = vm.ActivePrecompiles(rules)

	// Intrinsic costs are the only things reduced from initial gas to this point
	intrinsicGas, err := IntrinsicGas(input, jst.ctx["type"] == "CREATE")
	if err != nil {
		return
	}
...
func IntrinsicGas(data []byte, isContractCreation bool) (uint64, error) {
	...
	if len(data) > 0 {
		...
		if (math.MaxUint64-gas)/uint64(4) < z {
			return 0, errors.New("gas uint64 overflow")
		}
		gas += z * uint64(4)
	}
...
func (jst *jsTracer) CaptureState(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, rData []byte, depth int, err error) {
	if !jst.traceSteps {
		return
	}
	if jst.err != nil {
	...
	jst.opWrapper.op = evm.OpCode(op)
	...
	*jst.refundValue = uint((*jst.transition.Txn()).GetRefund())
...
func (jst *jsTracer) CaptureFault(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, depth int, err error) {
	if jst.err != nil {
		return
	}
...
func (jst *jsTracer) CaptureEnter(typ int, from types.Address, to types.Address, input []byte, gas uint64, value *big.Int) {
	...
	*jst.frame.typ = evm.OpCode(typ).String()
	*jst.frame.from = from
	*jst.frame.to = to
	jst.frame.input = types.CopyBytes(input)
...
func (jst *jsTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	jst.frameResult.output = types.CopyBytes(output)
