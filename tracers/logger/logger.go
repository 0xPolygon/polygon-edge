...
func (l *StructLogger) GetResult() (json.RawMessage, error) {
	...
	// if failed && l.err != evm.ErrExecutionReverted {
	// 	returnVal = ""
	// }
	if failed {
		returnVal = ""
...
func (t *mdLogger) CaptureStart(txn interface{}, from types.Address, to types.Address, create bool, input []byte, gas uint64, value *big.Int) {
	// t.env = env
	if !create {
...
// CaptureState also tracks SLOAD/SSTORE ops to track storage change.
func (t *mdLogger) CaptureState(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, rData []byte, depth int, err error) {
	stack := scope.Stack
	...
	fmt.Fprintf(t.out, "%10v |", t.txn.GetRefund())
	fmt.Fprintln(t.out, "")
	if err != nil {
...
func (t *mdLogger) CaptureFault(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, depth int, err error) {
...
func (t *mdLogger) CaptureEnter(typ int, from types.Address, to types.Address, input []byte, gas uint64, value *big.Int) {
}
