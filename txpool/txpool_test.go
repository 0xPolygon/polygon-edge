...
func TestAddingTransaction(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			pool, err := NewTxPool(hclog.NewNullLogger(), false, nil, false, defaultPriceLimit, defaultMaxSlots, forks.At(0), &mockStore{}, nil, nil, nil)
			if err != nil {
...
func TestMultipleTransactions(t *testing.T) {
	...
	pool, err := NewTxPool(hclog.NewNullLogger(), false, nil, true, defaultPriceLimit, defaultMaxSlots, forks.At(0), &mockStore{}, nil, nil, nil)
...
func TestBroadcast(t *testing.T) {
	...
	createPool := func() (*TxPool, *network.Server) {
		...
		pool, err := NewTxPool(hclog.NewNullLogger(), false, nil, true, defaultPriceLimit, defaultMaxSlots, forks.At(0), &mockStore{}, nil, server, nil)
		assert.NoError(t, err)
		pool.AddSigner(signer)
		return pool, server
	}
...
func TestTxPool_ErrorCodes(t *testing.T) {
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			pool, err := NewTxPool(hclog.NewNullLogger(), false, nil, true, defaultPriceLimit, defaultMaxSlots, forks.At(0), testCase.mockStore, nil, nil, nil)
			assert.NoError(t, err)
			if testCase.devMode {
				pool.EnableDev()
			}
...
func TestPriceLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pool, err := NewTxPool(hclog.NewNullLogger(), false, tt.locals, tt.noLocals, tt.priceLimit, defaultMaxSlots, forks.At(0), &mockStore{}, nil, nil, nil)
			assert.NoError(t, err)
			pool.AddSigner(signer)
...
func TestSizeLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pool, err := NewTxPool(hclog.NewNullLogger(), false, nil, false, defaultPriceLimit, tt.maxSlot, forks.At(0), &mockStore{}, nil, nil, nil)
			assert.NoError(t, err)
			pool.AddSigner(signer)
