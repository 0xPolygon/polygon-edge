...
func (i *backendIBFT) InsertBlock(proposal []byte, committedSeals []*messages.CommittedSeal) {
	...
	header, err := i.signer.WriteCommittedSeals(newBlock.Header, committedSealsMap)
	if err != nil {
	...
	i.logger.Info(
		...
		"hash", newBlock.Hash(),
		"validators", i.activeValidatorSet.Len(),
...
func (i *backendIBFT) ID() []byte {
	return i.signer.Address().Bytes()
}

func (i *backendIBFT) MaximumFaultyNodes() uint64 {
	return uint64(CalcMaxFaultyNodes(i.activeValidatorSet))
}
...
func (i *backendIBFT) buildBlock(snap *Snapshot, parent *types.Header) (*types.Block, error) {
	...
	header, err = i.signer.WriteProposerSeal(header)
	if err != nil {
		return nil, err
	}
