...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run("", func(t *testing.T) {
			...
			extra, _ := prepareExtra(genesis, validators)
			...
			*/
			...
			engine := New(config, accounts.accounts[tt.validators[0]], dbRaw, logger)
			...
			for j, vote := range tt.votes {
				...
				copy(headers[j].ExtraData, blockchain.GetGenesis().ExtraData)
			...
			if err != nil {
				t.Fatalf("test %d: failed to create voting snapshot: %v", i, err)
			}
			...
			if len(result) != len(validators) {
				t.Fatalf("test %d: validators mismatch: have %x, want %x", i, result, validators)
			}
			for j := 0; j < len(result); j++ {
				if !bytes.Equal(result[j][:], validators[j][:]) {
					t.Fatalf("test %d, validator %d: validator mismatch: have %x, want %x", i, j, result[j], validators[j])
				}
			}
		})
	}
...
func TestSaveAndLoad(t *testing.T) {
	...
	engine := &backend{
		db: memory.NewMemoryRaw(),
	}
	if err := engine.WriteSnapshot(snap); err != nil {
		t.Fatal(err)
	}

	snap1, err := engine.LoadSnapshot(snap.Epoch, snap.Hash)
	if err != nil {
