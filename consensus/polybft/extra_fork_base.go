...
func init() {
	// for tests
	forkManagerInstance.RegisterFork(forkmanager.InitialFork, nil)
...
func RegisterExtraDataForkHandlers() error {
	if err := forkManagerInstance.RegisterHandler(
		forkmanager.InitialFork, extraHandler, &ExtraHandlerBase{}); err != nil {
		return err
	}

	if forkManagerInstance.IsForkRegistered(chain.DoubleSignSlashing) {
		if err := forkManagerInstance.RegisterHandler(
			chain.DoubleSignSlashing, extraHandler, &ExtraHandlerSlashing{}); err != nil {
			return err
		}
	}
