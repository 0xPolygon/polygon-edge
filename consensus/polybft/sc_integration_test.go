...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// call commit epoch
		result := transition.Call2(contracts.SystemCaller, contracts.ValidatorSetContract, input, big.NewInt(0), 10000000000, runtime.NewAccessList())
		require.NoError(t, result.Err)
		...
		result = transition.Call2(contracts.SystemCaller, contracts.RewardPoolContract, input, big.NewInt(0), 10000000000, runtime.NewAccessList())
		require.NoError(t, result.Err)
...
func deployAndInitContract(t *testing.T, transition *state.Transition, scArtifact *artifact.Artifact, sender types.Address,
	...
	deployResult := transition.Create2(sender, scArtifact.Bytecode, big.NewInt(0), 1e9, runtime.NewAccessList())
