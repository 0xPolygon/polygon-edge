...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	currentEpochNumber := getEpochNumber(epochSize, epochSize)
	newEpochNumber := currentEpochNumber + 1
	extraParent := &Extra{
		Checkpoint: &CheckpointData{},
	}
	extra := &Extra{
		Validators: &ValidatorSetDelta{Added: validatorSet},
		Checkpoint: &CheckpointData{},
	}
	parentBlock := consensus.BuildBlock(consensus.BuildBlockParams{
		Header: &types.Header{
			Number:    epochSize - 1,
			ExtraData: append(make([]byte, ExtraVanity), extraParent.MarshalRLPTo(nil)...),
		},
	})
	...
	blockchainMock.On("GetHeaderByNumber", mock.Anything).Return(parentBlock.Header, true).Once()
	...
	runtime := &consensusRuntime{
		proposerSnapshot: NewProposerSnapshot(epochSize-1, validatorSet),
		logger:           hclog.NewNullLogger(),
		state:            newTestState(t),
		config: &runtimeConfig{
			PolyBFTConfig: &PolyBFTConfig{
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	runtime := &consensusRuntime{
		...
		state:            newTestState(t),
		proposerSnapshot: NewProposerSnapshot(blockNumber, []*ValidatorMetadata{}),
		config: &runtimeConfig{
...
func TestConsensusRuntime_FSM_NotInValidatorSet(t *testing.T) {
	...
	runtime := &consensusRuntime{
		proposerSnapshot:    NewProposerSnapshot(1, nil),
		activeValidatorFlag: 1,
		config: &runtimeConfig{
			proposerCalc: NewProposerCalculator(hclog.NewNullLogger()),
			PolyBFTConfig: &PolyBFTConfig{
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	blockchainMock.On("CurrentHeader").Return(&types.Header{Number: 1})
	blockchainMock.On("GetStateProviderForBlock", mock.Anything).Return(new(stateProviderMock)).Once()
	blockchainMock.On("GetSystemState", mock.Anything, mock.Anything).Return(systemStateMock).Once()

	polybftBackendMock := new(polybftBackendMock)
	polybftBackendMock.On("GetValidators", uint64(0), mock.Anything).Return(validators.getPublicIdentities()).Once()
	polybftBackendMock.On("GetValidators", uint64(1), mock.Anything).Return(validators.getPublicIdentities()).Once()
...
func TestConsensusRuntime_restartEpoch_FirstRestart_BuildsCommitment(t *testing.T) {
	...
	blockchainMock.On("GetStateProviderForBlock", mock.Anything).Return(new(stateProviderMock)).Once()
	blockchainMock.On("GetSystemState", mock.Anything, mock.Anything).Return(systemStateMock).Once()
	blockchainMock.On("GetHeaderByNumber", header.Number).Return(&types.Header{
	...
	runtime := &consensusRuntime{
		proposerSnapshot:    NewProposerSnapshot(header.Number, validators),
		logger:              hclog.NewNullLogger(),
		activeValidatorFlag: 1,
		state:               state,
		config: &runtimeConfig{
...
func TestConsensusRuntime_restartEpoch_NewEpochToRun_BuildCommitment(t *testing.T) {
	...
	runtime := &consensusRuntime{
		proposerSnapshot:    NewProposerSnapshot(header.Number, nil),
		logger:              hclog.NewNullLogger(),
		activeValidatorFlag: 1,
		state:               state,
		config: &runtimeConfig{
...
func TestConsensusRuntime_FSM_EndOfEpoch_OnBlockInserted(t *testing.T) {
	...
	runtime := &consensusRuntime{
		proposerSnapshot:  NewProposerSnapshot(numOfBlock, nil),
		logger:            hclog.NewNullLogger(),
		state:             state,
		epoch:             metadata,
		config:            config,
...
func TestConsensusRuntime_IsValidSender(t *testing.T) {
	...
	runtime := &consensusRuntime{
		...
		logger:           hclog.NewNullLogger(),
		proposerSnapshot: NewProposerSnapshot(0, nil),
	}
...
func TestConsensusRuntime_HasQuorum(t *testing.T) {
	...
	runtime := &consensusRuntime{
		...
		logger:           hclog.NewNullLogger(),
		proposerSnapshot: NewProposerSnapshot(lastBuildBlock.Number+1, validatorAccounts.getPublicIdentities()),
	}
	...
	runtime.fsm.proposerSnapshot = runtime.proposerSnapshot
