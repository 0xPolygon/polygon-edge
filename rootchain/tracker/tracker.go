...
func NewEventTracker(logger hclog.Logger, eventConfig *rootchain.ConfigEvent, rootchainWS string) (*EventTracker, error) {
	if eventConfig == nil {
...
func (t *EventTracker) encodeEventFromLog(log *ethgo.Log) (rootchain.Event, error) {
	...
	case rootchain.ValidatorSetPayloadType:
		return t.encodeEventForValidatorSetPayload(eventData, log.BlockNumber)
	}

	return rootchain.Event{}, errors.New("cannot parse event, no payloadType")
}

// encodeEventFromLog encodes event for specific payload
func (t *EventTracker) encodeEventForValidatorSetPayload(eventData map[string]interface{}, blockNumber uint64) (rootchain.Event, error) {
	var (
		index *big.Int
		ok    bool
